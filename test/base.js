...
for (const vector of primaryKeyVectors) {
  test('should pass test vector: ' + vector.description, t => {
    ...
    t.alike(defaultKeyPair?.publicKey, b4a.from(vector.default, 'hex'))

    const namedKeyPair = KeyChain.createKeyPair(primaryKey, vector.name)
    t.alike(namedKeyPair.publicKey, b4a.from(vector.key, 'hex'))
  })
}
